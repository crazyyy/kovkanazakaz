    ...
    if ($('#offers-form-container')){
        ...
        $subscribeForm.on('change keyup focusout blur', function() {
            if($(this).validate().checkForm()) {
                $subscribeForm.find('input[type=submit]').eq(0).parent().addClass('button-container-inline');
                $subscribeForm.find('input[type=submit]').eq(0).removeClass('btn-disabled').attr('disabled', false);
            } else {
                $subscribeForm.find('input[type=submit]').eq(0).parent().removeClass('button-container-inline');
